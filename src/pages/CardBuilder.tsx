...
interface CardConfig {
  ...
  isPublic: boolean;
  stampingDelay: number; // minutes, 0 = off
}

interface Reward {
  id: string;
  type: 'main' | 'signup' | 'interim';
  name: string;
  nameAr: string;
...
function PhoneMockup({ config }: { config: CardConfig }) {
...
function Field({ label, value, onChange, placeholder, dir, type, textarea, required }: {
  ...
  dir?: 'ltr' | 'rtl'; type?: string; textarea?: boolean; required?: boolean;
}) {
  const shared = {
    value,
    ...
    style: {
      ...
      minHeight: textarea ? 80 : 'auto', resize: textarea ? 'vertical' as const : 'none' as const,
...
export default function CardBuilder() {
  ...
  const updateConfig = (updates: Partial<CardConfig>) => setConfig(prev => ({ ...prev, ...updates }));
  ...
  const renderRewardsStep = () => {
    const addReward = (type: 'main' | 'signup' | 'interim') => {
      updateConfig({
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          {(['main', 'signup', 'interim'] as const).map(type => {
